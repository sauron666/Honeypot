...
> Легенда: ✅ готово · ◐ частично · без маркер = предстои
...
- [x] ✅ `ObserverDriver`: `none` + `drakvuf` (parsing/mapping готови, hypervisor-glue остава) — glue завършен, live-validated на Windows Server 2025 и Linux (Xen)
...
- [x] ✅ Golden template: `deb12-web` (Packer + cloud-init за Proxmox/libvirt) — реализиран като `templates/ubuntu2404-cloud` (Ubuntu 24.04), field-proven на PVE 8.4
...
- [x] ✅ `ComputeDriver: proxmox` (Proxmox VE през pvesh CLI) — вече и REST API драйвер
...
- [x] ✅ `ObserverDriver`: DRAKVUF parsing/mapping тествано; hypervisor-glue остава (ADR-010) — glue завършен, live-validated (Windows Server 2025 пълна интроспекция; Linux guest listing)
- [x] ✅ SMB2: negotiate, session setup (NetNTLMv2 улов), tree connect + **файлови операции** (Create/Read/Write/Close/QueryDirectory/QueryInfo) — wire-нати в `smb_files.go`, тествани
...
- [x] ✅ Ransomware детекция: 6 сигнала + tarpit + извличане на контакти — храни се от FTP дяла и **hypervisor-agnostic FUSE trap** (`internal/fusetrap`); SMB записите се записват, не се скорират
...
- [ ] ◐ Crypto key capture от паметта — нужен VMI — crypto hook (apimon/BCryptEncrypt→T1486) + DumpMemory готови; изисква Xen dom0 с VMFUNC
...
- [x] ✅ `mirage-vault`: hash chain, RFC3161, подписан evidence package — завършен (`internal/vault`): ed25519 seal + RFC 3161 timestamp
- [ ] Локален LLM аналитик (офлайн) — отложен
- [ ] Multi-tenancy, SSO/SAML — отложен
- [ ] ◐ `ComputeDriver: vsphere/hyper-v`; `IdentityDriver: Entra ID/Okta` — vsphere/hyperv структура готова (experimental, unit-тествани, чакат жива валидация); Entra ID/Okta предстоят
